...
-- Register the command to play a scenario
RegisterCommand("hc.playScenario", function(source, args, rawCommand)
    ...
    local scenarioIndex = tonumber(args[1])
    if scenarioIndex and scenarioIndex >= 1 and scenarioIndex <= #scenarios then
        if playerPed then
            local scenario = scenarios[scenarioIndex]
            TaskStartScenarioInPlace(playerPed, scenario, 0, true)
            print("Playing scenario: " .. scenario)
        else
            print("Failed to get player ped")
        end
    else
        print("Invalid scenario index. Please provide a number between 1 and " .. #scenarios)
    end
end, false)