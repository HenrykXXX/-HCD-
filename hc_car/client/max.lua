...
function maxTuneVehicle(multiplier, traction)
	...
	local tractionMultiplier = tonumber(traction) or 1.0
        SetVehicleHandlingFloat(vehicle, "CHandlingData", "fTractionCurveMax", tractionMultiplier)
    else
...
RegisterCommand('max', function(source, args, rawCommand)
    ...
    local traction = args[2] -- Get traction multiplier

    maxTuneVehicle(multiplier, traction)
end, false)